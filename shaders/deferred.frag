...
vec3 calcDirLight(DirectionalLight light, vec3 albedo, float metallic, float roughness, float ao, vec3 normal,
                  vec3 view, vec3 position) {
    vec3 F0 = vec3(0.04);
    F0 = mix(F0, albedo, metallic);
    vec3 light_dir = normalize(light.direction);
    vec3 H = normalize(view + light_dir);
    vec3 radiance = light.color *  LIGHT_MULTIPLIER;
...
vec3 calcPointLight(PointLight light, vec3 albedo, float metallic, float roughness, float ao, vec3 normal, vec3 view,
                    vec3 position) {
    vec3 F0 = vec3(0.04);
    F0 = mix(F0, albedo, metallic);
...
vec3 calcSpotLight(SpotLight light, vec3 albedo, float metallic, float roughness, float ao, vec3 normal, vec3 view,
                   vec3 position) {
    vec3 light_dir = normalize(light.position - position);
