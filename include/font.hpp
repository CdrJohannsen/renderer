    ...
    void drawString(float x, float y,const char* text, Shader* fontShader ){
        ...
        while(*text){
            if (*text >= 32 && *text < 128){
                ...
                vData[0].position = glm::vec2(q.x0,q.y0); vData[0].texCoord = glm::vec2(q.s0,q.t0);
                vData[1].position = glm::vec2(q.x1,q.y0); vData[1].texCoord = glm::vec2(q.s1,q.t0);
                vData[2].position = glm::vec2(q.x1,q.y1); vData[2].texCoord = glm::vec2(q.s1,q.t1);
                vData[3].position = glm::vec2(q.x0,q.y1); vData[3].texCoord = glm::vec2(q.s0,q.t1);
                vData[4].position = glm::vec2(q.x0,q.y0); vData[4].texCoord = glm::vec2(q.s0,q.t0);
                vData[5].position = glm::vec2(q.x1,q.y1); vData[5].texCoord = glm::vec2(q.s1,q.t1);
