...
void GBuffer::create(uint32_t w, uint32_t h) {
    width = w;
    heigth = h;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, heigth, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, width, heigth, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        cout << "[WARNING]: GBuffer creation failed" << endl;
    }
...
void GBuffer::destroy() { glDeleteFramebuffers(1, &fbo); }
...
void GBuffer::blitFramebuffer() {
    ...
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER,
                      0);  // write to default framebuffer
                           ...
                           // both the FBO and default framebuffer have to match. the internal formats are
                           // implementation defined. This works on all of my systems, but if it doesn't on yours you'll
                           // likely have to write to the depth buffer in another shader stage (or somehow see to match
                           // the default framebuffer's internal format with the FBO's internal format).
